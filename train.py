...
DATA_NAME = 'spinodal_28_v2reg'  # spinodal_28, mnist
RUN_MODE = 'train_latent2params'  # train_anvae, train_latent2params
...
def plot_reconstruction(model, data, epoch, step):
    ...
    latentSpace = np.array(model.image2latent(data))
    numpyName = '{}/latent_{}_{}_{}.npy'.format(IMG_DIR, RUN_NAME, epoch, step)
    np.save(numpyName, latentSpace)


def plot_latent_space(model, data, epoch, step):
    latentSpace = model.image2latent(data)
    print("latentDims: ", latentSpace.shape)
    #fig, axs = plt.imshow(latentSpace)

    latentNp = np.array(latentSpace)
    np.save("latentSpace.npy",latentNp)

    img = np.array(latentSpace[0,:,:,0])
    print(img)

    #plt.savefig('{}/latent_{}_{}_{}.png'.format(IMG_DIR, RUN_NAME, epoch, step))
    cv2.imwrite("latent.png",img*255)
...
if RUN_MODE == 'train_anvae':
    ...
    for epoch in range(N_EPOCHS):
        ...
        while epoch_completed == False:
            ...
            if (train_model.step_count % 200 == 0):
                ...
                #plot_latent_space(train_model, image_batch, epoch, iteration)
    ...
    if DATA_NAME.endswith('reg'):
        ...
        np.save('data/zmeans.npy', zmeans)
        np.save('data/ytest.npy', y_test)
        np.save('data/x_test_latent.npy', x_test_latent)
